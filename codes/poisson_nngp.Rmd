...
The goal here is to find the parameters of the multivariate normal distribution \eqref{joint density of nngp}. In order to obtain that, we write the conditional densities as a linear combination of normal distributions $\gamma_i \sim \mathcal{N}(0, d_i)$
...
where $\boldsymbol{A} = (a_{ij})$ is a lower triangular matrix and $\boldsymbol{\gamma} \sim \mathcal{N}(\boldsymbol{0}, \boldsymbol{D})$ with $\boldsymbol{D} = diag(d_1, \dots, d_n)$. Finally we have the specification of \eqref{joint density of nngp}
...
Which has a sparse precision matrix with Cholesky decomposition $\boldsymbol{L}^T \boldsymbol{L}$
...
## NNGP: Find the neighbourhood of each location

For simplicity we are using the function `spConjNNGP()` from the package `spNNGP` to get the neighbourhood of each location.
...
The code bellow shows the neighbours of the locations 2 to 6.
...
To compute the sparse precision matrix we must compute the matrices $\boldsymbol{A}$ and $\boldsymbol{D}$ using \eqref{conditional densities of nngp}. The function `nngp_chol()` compute the mentioned matrices and the Cholesky decomposition of the precision matrix.

The main idea of the algorithm is to initialise $\boldsymbol{A} = (\boldsymbol{a}_1, \dots, \boldsymbol{a}_n)$ as matrix of zeros and the vector $\boldsymbol{d} = diag(\boldsymbol{D})$ with $d_1 = C(\boldsymbol{S}_1, \boldsymbol{S}_1)$ then for each location $i = 2, \dots, n$ we update the matrix and vector according to the neighbours of $i$.
...
For example let's calculate for location of index 8. Assuming known parameters, first we initialise $\boldsymbol{A}$ and $\boldsymbol{d}$ (named `dd` in the code bellow); and obtain the neighbours for each point.
...
Then we compute the covariance matrix between the location 8 and its neighbours considering the location 8 in the first position of the matrix.
...
Considering the neighbourhood of 8 use \eqref{conditional densities of nngp} to compute $\boldsymbol{a}_8$ and $d_8$ and update $\boldsymbol{A}$ and $\boldsymbol{d}$.
...
Computing for locations $i = 2, \dots, n$ then we have the matrix $\boldsymbol{A}$ and $\boldsymbol{D}$ which can the sparsity can be visualise as follows in the plots bellow, where black represents non zero values.
...
nngp_decomp <- nngp_chol(coords, m, c(sigma, l))
...
We can compare the Full GP (original) precision matrix and the NNGP version. Obviously $\tilde{C}^{-1}$ becomes more sparse as $n$ increase and $m$ is small.
...
For simplicity let's show the approximation for a univariate setting $y \mid x \sim Poisson(\eta)$ and $x \sim \mathcal{N}(0, \sigma^2)$ where $\eta = \exp(x)$ and the parameter is known.
...
We have to define the log posterior of the parameters, its gradient and hessian function.
