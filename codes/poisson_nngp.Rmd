...
\begin{equation}
    x_i \mid \boldsymbol{x}_{\boldsymbol{c}(i)} \sim \mathcal{N} \left( \boldsymbol{a}_i \boldsymbol{x}_{\boldsymbol{c}(i)}, \boldsymbol{D}_i \right) \quad
    \begin{cases}
        \boldsymbol{a}_i = C(s_i, \boldsymbol{c}(i)) \left(C(\boldsymbol{c}(i), \boldsymbol{c}(i) \right)^{-1} \\
        \boldsymbol{D}_i = C(s_i, s_i) - \boldsymbol{a}_i^T C(\boldsymbol{c}(i), \boldsymbol{s}_i)
    \end{cases}.
...
Which generates a sparse precision matrix that has a Cholesky decomposition $\boldsymbol{L}^T \boldsymbol{L}$
...
As mentioned to compute the sparse precision matrix we must compute the matrices $\boldsymbol{A}$ and $\boldsymbol{D}$ using \eqref{conditional densities of nngp}. The function \texttt{nngp\_chol()} compute the mentioned matrices and the Cholesky decomposition of the precision matrix.

The main idea of the algorithm is to initialise $\boldsymbol{A} = (\boldsymbol{a}_1, \dots, \boldsymbol{a}_n)$ as matrix of zeros and the vector $\boldsymbol{d} = diag(\boldsymbol{D})$ with $d_1 = C(\boldsymbol{S}_1, \boldsymbol{S}_1)$ then for each location $i = 2, \dots, n$ we update the the matrix and vector according to the neighbours of $i$.
...
Let's look the update for location 8. Assuming a known parameters, first we initialise $\boldsymbol{A}$ and $\boldsymbol{d}$ (named \texttt{dd} in the code bellow); and obtain the neighbours for each point.
...
Computing for locations $i = 2, \dots, n$ then we have the matrix $\boldsymbol{A}$ and $\boldsymbol{D}$ which can the sparsity can be visualised as follows in the plots bellow, where black represents non zero values.
